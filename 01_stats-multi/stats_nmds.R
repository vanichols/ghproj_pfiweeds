##################################
# Author: Lydia English (lenglish@iastate.edu), Gina trying to understand
# Created: Jan 24 2019
#
...
dat_div <- 
  ...
  ungroup() %>% 
  # calculate diversity
  mutate(shan = vegan::diversity(.[, 7:24]),
         shan_hill = exp(shan)) %>%
  unite("site_sys", site_name, sys_trt, sep = "_",remove = FALSE) 
