...
# updated 8/31/2020, remove species with very small %s
...
# which species appeared in every site?
pfi_weedsplist

# which ones DIDN'T appear in every site?
not0 <- 
  pfi_ghobsraw %>% 
  pfifun_sum_weedbyeu() %>% 
  filter(seeds != 0) %>% 
  unite(site_name, field, sys_trt, col = "site_id") 
...
not0_4 <- 
  not0 %>% 
  filter(site_id == sites[4]) %>% 
  select(site_id, weed)  %>% 
  distinct() %>% 
  rename("s4" = "site_id")

top4  <- 
  left_join(not0_1, not0_2) %>% 
  left_join(not0_3) %>% 
  left_join(not0_4) %>% 
  filter(!is.na(s2)) %>% 
  filter(!is.na(s4)) %>% 
...
#--keep only those that appeared in all 4?
df_dat <- 
  pfi_ghobsraw %>%
...
site_scores %>% 
  write_csv("01_stats-nmds/st_nmds-site-top4.csv")
...
spp_scores %>% 
  write_csv("01_stats-nmds/st_nmds-spp-top4.csv")
...
site_hull %>% 
  write_csv("01_stats-nmds/st_nmds-site-hulls-top4.csv")
