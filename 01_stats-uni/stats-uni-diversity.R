...
# i. diversity
# fixed effects model 
l1 <- lmer(shan_hill ~ site_sys*cc_trt + (1|blockID), data = div_rich)
summary(l1)
anova(l1)
...
r1 <- lmer(richness ~ site_sys*cc_trt + (1|blockID), data = div_rich)
summary(r1)
anova(r1)   # significant interaction term since richness increased for Funcke and Stout
emmeans(r1, pairwise ~ cc_trt|site_sys, type = "response")
# interestingly boyd richness is significanly lower in silage treatment...


# iii. evenness

r1 <- lmer(evenness ~ site_sys*cc_trt + (1|blockID), data = div_rich)
summary(r1)
anova(r1)   # significant interaction term since richness increased for Funcke and Stout
emmeans(r1, pairwise ~ cc_trt|site_sys, type = "response")
# interestingly boyd richness is significanly lower in silage treatment...
