...
read_csv("../01_stats-uni/st_weedseed-contr-all-models.csv") %>% 
  ...
  mutate_all(funs(str_replace(., "Boyd", "Central"))) %>% 
  mutate_all(funs(str_replace(., "Funcke", "West"))) %>%
  mutate_all(funs(str_replace(., "Stout", "East"))) %>% 
...
```{r dotplot, fig.cap="Manuscript fig. 3 using full dataset"}
